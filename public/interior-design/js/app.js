...
"f90da5e3-9e0e-eba7-173d-eb0b071e838e,71d4f128-ae80-3d58-9bd2-711c6ce6cdf2,4e3d65cb-54c0-0681-28bf-bddcc7bdb571,da026c08-d76a-a944-8e7b-096b752da9ed":{"name":"A New Room"}\
},"wallTextures":[],"floorTextures":{},"newFloorTextures":{},"carbonSheet":{"url":"","transparency":1,"x":0,"y":0,"anchorX":0,"anchorY":0,"width":0.01,"height":0.01}},"items":[]}'
...
var mainControls = function (blueprint3d) {
  ...
  function saveDesign() {
    ...
    // Set up the save button handler - we define it here to avoid duplicate bindings
    $("#confirm-save-btn").off('click').on('click', function() {
      var designName = $("#design-name").val().trim();
      ...
      localStorage.setItem("design_" + designName, data);
      ...
      showAutoSaveStatus('Design "' + designName + '" saved successfully!');
    });
    
    // Handle Enter key in the input field
    $("#design-name").off('keypress').on('keypress', function(e) {
      if (e.which === 13) { // Enter key
        $("#confirm-save-btn").click();
      }
    });
  }
  ...
  function init() {
    ...
    // Remove handlers for old buttons that have been removed
    ...
    // Make sure design buttons are properly connected
    $("#saveDesignBtn").click(saveDesign);
    $("#loadDesignBtn").click(handleLoadDesign);
    $("#importDesignBtn").click(handleImportDesign);
...
$(document).ready(function () {
  ...
  // Fix the currentWall reference by checking if aWall exists
  $("#add-items")
    .find(".add-item")
    .mousedown(function (e) {
      ...
      // Check if aWall exists and has currentWall property
  ...
  // var saveLoadDiv = $(
  //   '<div class="save-load-buttons" style="position: fixed; bottom: 20px; right: 20px; z-index: 1000;"></div>'
  // );

  // var saveBtn = $(
  //   '<button class="btn btn-primary" style="margin-right: 10px;">Save Design</button>'
  // );
  // var loadBtn = $(
  //   '<button class="btn btn-primary" style="margin-right: 10px;">Load Design</button>'
  // );
  // var importBtn = $('<button class="btn btn-primary">Import Design</button>');

  // saveBtn.click(saveDesign);
  // loadBtn.click(loadDesign);
  // importBtn.click(importDesign);

  // saveLoadDiv.append(saveBtn);
  // saveLoadDiv.append(loadBtn);
  // saveLoadDiv.append(importBtn);

  // $("body").append(saveLoadDiv);
});
...
function loadDesign() {
  ...
  for (var i = 0; i < localStorage.length; i++) {
    ...
    if (key.startsWith("design_")) {
      designs.push(key.replace("design_", ""));
    }
  }
  ...
  var designName = prompt(
    "Enter the name of the design to load:\nAvailable designs: " +
      designs.join(", ")
  );
  if (designName) {
    var data = localStorage.getItem("design_" + designName);
    if (data) {
      blueprint3d.model.loadSerialized(data);
...
function initFloorplanner() {
  var canvas = document.getElementById("floorplanner");
  var floorplan = new BP3DJS.Model.Floorplan();
  var floorplanner = new BP3DJS.Floorplanner.Floorplanner2D(
    "floorplanner",
    floorplan
  );

  // Initialize the floorplanner view
  var viewerFloorplanner = new ViewerFloorplanner({
    floorplanner: floorplanner,
    model: floorplan,
  });

  // Add event listeners for wall and room operations
  floorplanner.addEventListener(BP3DJS.Events.WALL_SELECTED, function (event) {
    var wallProperties = new WallProperties();
    wallProperties.setWall(event.wall);
    wallPropFolder = getWallPropertiesFolder(gui, wallProperties);
    wallPropFolder.open();
  });

  floorplanner.addEventListener(BP3DJS.Events.ROOM_SELECTED, function (event) {
    var roomProperties = new RoomProperties(event.room, gui);
    floorPropFolder = getFloorPropertiesFolder(gui, roomProperties);
    floorPropFolder.open();
  });

  // Set default wall height and ceiling configuration
  BP3DJS.Configuration.setValue(BP3DJS.configWallHeight, 250); // 250cm default wall height

  // Initialize with default floorplan
  floorplan.loadSerialized(myhome);

  // Force redraw to ensure proper ceiling generation
  floorplanner.view.draw();

  // Add resize handler
  $(window).resize(function () {
    floorplanner.handleWindowResize();
  });
}

var FurnitureResizer = function(blueprint3d) {
    var scope = this;
    ...
    function init() {
        console.log("Initializing FurnitureResizer");
        
        // Create resize button
        resizeButton = $('<button class="resize-button">Resize</button>')
            .css({
                'position': 'fixed',
                'bottom': '20px',
                'left': '20px',
        ...
        $('body').append(resizeButton);

        // Create resize helper geometry
        ...
        resizeHelper = new THREE.Mesh(geometry, material);
        resizeHelper.visible = false;
        three.scene.add(resizeHelper);
        console.log("Resize helper added to scene:", resizeHelper);
        ...
        three.addEventListener(BP3DJS.EVENT_ITEM_SELECTED, handleItemSelected);
        three.addEventListener(BP3DJS.EVENT_ITEM_UNSELECTED, handleItemUnselected);
    }
    ...
    function toggleResizeMode() {
        ...
        if (!isResizing) {
            // Enter resize mode
            isResizing = true;
            resizeButton.css('background-color', '#f44336');
            resizeButton.text('Stop Resizing');
            updateResizeHelperPosition();
        } else {
            // Exit resize mode
            isResizing = false;
            resizeButton.css('background-color', '#4CAF50');
    ...
    function handleItemSelected(event) {
        ...
        originalSize = {
            width: selectedItem.getWidth(),
            height: selectedItem.getHeight(),
    ...
    function updateResizeHelperPosition() {
        if (!selectedItem || !resizeHelper) return;
        
        console.log("Updating resize helper position");
        // Position helper at the corner of the item
        ...
        selectedItem.getWorldPosition(itemWorldPos);
        
        // Position the helper at the top-right-front corner
        resizeHelper.position.copy(itemWorldPos)
            .add(new THREE.Vector3(size.x/2, size.y/2, size.z/2));
        
        resizeHelper.visible = true;
        console.log("Resize helper position:", resizeHelper.position);
    }
    ...
    function onMouseDown(event) {
        ...
        // Check for intersection with resize helper
        var intersects = raycaster.intersectObject(resizeHelper);
        console.log("Mouse down, intersects:", intersects.length);
        
        if (intersects.length > 0) {
            isResizing = true;
            resizeAxis = new THREE.Vector3(1, 1, 1); // Resize in all directions
            console.log("Resize started");
        }
    }

    function onMouseMove(event) {
        if (!isResizing || !selectedItem || !resizeHelper) return;
        ...
        var distance = intersection.distanceTo(selectedItem.position);
        var originalDistance = resizeHelper.position.distanceTo(selectedItem.position);
        var scale = distance / originalDistance;

        // Apply resize
        var newWidth = originalSize.width * scale;
        var newHeight = originalSize.height * scale;
        var newDepth = originalSize.depth * scale;

        console.log("Resizing to:", { width: newWidth, height: newHeight, depth: newDepth });

        // Update item size
        selectedItem.resize(newHeight, newWidth, newDepth);
        updateResizeHelperPosition();
    }
...
// Function to show auto-save status
function showAutoSaveStatus(message) {
  var statusElement = document.getElementById('auto-save-status');
  if (statusElement) {
...
function autoSaveDesign() {
  if (blueprint3d && blueprint3d.model) {
    ...
    console.log('Design auto-saved at', new Date().toLocaleString());
  }
}

// Function to display design management functionality
function manageDesigns() {
  ...
  for (var i = 0; i < localStorage.length; i++) {
    ...
    if (key.startsWith("design_")) {
      designs.push({
        name: key.replace("design_", ""),
        key: key,
        date: new Date().toLocaleString() // In a real app, we'd store timestamps with designs
      });
    }
  }

  // Clear previous entries
  $("#saved-designs-list").empty();
  
  if (designs.length === 0) {
    $("#no-designs-message").show();
    $("#saved-designs-list").hide();
  } else {
    $("#no-designs-message").hide();
    $("#saved-designs-list").show();
    
    // Add each design to the list
    designs.forEach(function(design) {
      var listItem = $('<a href="#" class="list-group-item"></a>');
      var header = $('<h4 class="list-group-item-heading"></h4>').text(design.name);
      var text = $('<p class="list-group-item-text text-muted"></p>').text("Last modified: " + design.date);
      
      listItem.append(header).append(text);
      
      // Add click handler to load the design
      listItem.click(function(e) {
        e.preventDefault();
        var data = localStorage.getItem(design.key);
        if (data) {
          // Load the design
          blueprint3d.model.loadSerialized(data);
          showAutoSaveStatus('Design "' + design.name + '" loaded successfully!');
          
          // Close the modal
          $("#load-design-modal").modal('hide');
        } else {
          alert("Error: Design data not found!");
        }
      });
      
      // Add delete button
      var deleteBtn = $('<button class="btn btn-xs btn-danger pull-right"><span class="glyphicon glyphicon-trash"></span></button>');
      deleteBtn.click(function(e) {
        ...
        if (confirm("Are you sure you want to delete the design \"" + design.name + "\"?")) {
          ...
          // Show "no designs" message if this was the last one
          if ($("#saved-designs-list").children().length === 0) {
            $("#no-designs-message").show();
            $("#saved-designs-list").hide();
          }
        }
      ...
      header.prepend(deleteBtn);
      $("#saved-designs-list").append(listItem);
    });
  }
  
  // Show the modal
  $("#load-design-modal").modal('show');
}

// Function to handle the loadDesignBtn click
function handleLoadDesign() {
  manageDesigns();
}

// Updated saveDesign function to handle button click
function saveDesign() {
  // Clear previous values and errors
  $("#design-name").val("");
  $("#save-error").hide();
...
// Function to handle the importDesignBtn click
...
// Enhanced init function to set up the design modals
$(document).ready(function() {
  // Fix the handleImportDesign function to use showAutoSaveStatus
  var originalHandleImportDesign = handleImportDesign;
  handleImportDesign = function() {
    var input = document.createElement("input");
    input.type = "file";
    input.accept = ".json,.blueprint3d";
      ...
      if (file) {
        ...
        reader.onload = function (e) {
          try {
            var data = e.target.result;
            blueprint3d.model.loadSerialized(data);
            showAutoSaveStatus('Design imported successfully!');
          } catch (error) {
            alert("Error importing design: " + error.message);
          }
        };
